function [est_shifts,shift_equations,est_scales,scale_equations]=cryo_estimate_shifts_scales_weighted(pf,rotations,...
    max_shift,shift_step,max_scale,Nscales,W,memoryfactor,shifts_2d_ref,verbose)
%
...
est_shifts = lscov(shift_equations(:,1:end-1),shift_equations(:,end),1./W);
...
est_scales1=lsqr(scale_equations(:,1:end-1),scale_equations(:,end),1.0e-8,100);
est_scales1=full(transpose(reshape(exp(est_scales1),1,n_projs)));
est_scales=lscov(scale_equations(:,1:end-1),scale_equations(:,end),1./W);
