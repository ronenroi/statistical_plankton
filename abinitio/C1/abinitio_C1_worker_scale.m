function abinitio_C1_worker_scale(instack,outparams,showfigs,...
...
[clstack] = only_commonlines_gaussian_shift_scale(pf,max_shift,Nshifts, Nscales, max_scale);%% Find only common lines from projections
...
indeces = clerr(:,1:2);
        errors = commonline_errors(W,indeces,size(projections,3));
		thr = 0.1;
        %%
        pf = pf(:,:,errors > thr);
        rotations = rotations(:,:,errors > thr);
        iterative_LS_est_scales = iterative_LS_est_scales(errors > thr);
        iterative_LS_est_shifts = iterative_LS_est_shifts(errors > thr,:);
        projections = projections(:,:,errors > thr);
        %%

log_message('Estimating shifts and scales');
    [est_shifts,~,est_scales,~]=cryo_estimate_shifts_scales_weighted(pf,rotations,max_shift,shift_step,max_scale,Nscales,clerr(:,3),10000,data.ref_shifts,0);
