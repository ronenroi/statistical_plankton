function [rots,in_plane_rots,max_corrs] = estimate_inplane_rotations_shift_scale(npf,vis,n_symm,inplane_rot_res,max_shift,shift_step,max_scale,Nscales)
%
% General description
%
% Input parameters:
...
shift_phases = gpuArray(calc_shift_phases(n_r,max_shift,shift_step));

max_corrs = zeros(nImages,nImages);
msg = [];
for i = 1:nImages
...
%             sqrt(sum((abs(scale_coeff)).^2)));
        ...
        %npf_i_shifted_scaled = zeros(n_r,n_theta,nshifts);
        npf_i_shifted_scaled = zeros([n_r,n_theta,nshifts],'gpuArray');

        for s=1:nshifts
             npf_i_shifted_scaled(:,:,s) = bsxfun(@times,scale_coeff,shift_phases(:,s));
        end
        
        npf_i_shifted_scaled(1,:) = 0; % ignoring dc term
        % normalize each ray to be of norm 1
        npf_i_shifted_scaled = bsxfun(@rdivide,npf_i_shifted_scaled,...
            sqrt(sum((abs(npf_i_shifted_scaled)).^2)));
        
        Npf_i_shifted_scaled = gpuArray(single(npf_i_shifted_scaled));
        %Npf_i_shifted_scaled = (single(npf_i_shifted_scaled));
        ...
        for j = i+1:nImages
           ...
           % Npf_j = (single(npf_j));
            ...
            co = bsxfun(@times,Npf_i_shifted_scaled(:,cijs,:),conj(Npf_j(:,cjis)));
            Corrs = sum(co);
            corrs = gather(Corrs);
            ...
            [tmp_max_corr,max_idx_corr] = max(mean(real(corrs),2));
            if tmp_max_corr > max_corrs(i,j)
                max_corrs(i,j) = tmp_max_corr;
