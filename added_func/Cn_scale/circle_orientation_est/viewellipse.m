function viewstack(projs,nrows,ncols,showlabels,centers,ellipsestack)
%
...
particles=draw(projs,1,nrows,ncols,showlabels,centers,ellipsestack);
sz=size(projs);
...
if nticks>1
    ...
    sld = uicontrol('Parent',gcf,'Style', 'slider',...
        ...
        'UserData',struct('projs',projs,'nrows',nrows,'ncols',ncols,'showlabels',showlabels,'centers',centers,'ellipsestack',ellipsestack),...
...
function particles=draw(projs,startidx,nrows,ncols,showlabels,centers,ellipsestack)
sz=size(projs);
borderwidth=2;
...
for k=1:ncols
    ...
    for j=1:nrows
        ...
        [xycenter] = round(centers(idx,:));
        ellipse = ellipsestack(:,:,idx);
        linearidx = sub2ind(size(ellipse),(xycenter(2)-3:xycenter(2)+3)',(xycenter(1)-3:xycenter(1)+3)');
        ellipse(linearidx) = 1;
        linearidx = sub2ind(size(ellipse),(xycenter(2)+3:-1:xycenter(2)-3)',(xycenter(1)-3:xycenter(1)+3)');
        ellipse(linearidx) = 1;
        
        img(rowloc:rowloc+sz(1)-1,colloc:colloc+sz(2)-1,1)=p+ ellipse;
...
draw(ud.projs,startidx,ud.nrows,ud.ncols,ud.showlabels,ud.centers,ud.ellipsestack);
end
