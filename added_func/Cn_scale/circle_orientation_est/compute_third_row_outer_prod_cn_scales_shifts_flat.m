function [vijs,viis,max_corrs_stats] = ...
    compute_third_row_outer_prod_cn_scales_shifts_flat...
    (npf,n_symm,max_shift,shift_step,max_scale,Nscales,inplane_rot_res,n_Points_sphere,projectionDir,refq)
...
if exist('projectionDir','var')
isDir = true;
else
    isDir = false;
    projectionDir=[];
end
[Ris_tilde,R_theta_ijs] = generate_cand_rots(n_Points_sphere,inplane_rot_res,isDir,projectionDir);
...
scales =[1, exp(linspace(-log(max_scale), log(max_scale), Nscales))];
...
% g_shift_phases = gpuArray(double(shift_phases));
 g_shift_phases = (double(shift_phases));
...
g_cijs_inds = (single(cijs_inds));
for i=1:nImages
    viis_tmp_i = zeros(3*nImages,3*nImages); % each pair of images will induce a candidate vi^vi^{T}. We keep them all and later average them all out
    vijs_i = zeros(3*nImages,3*nImages);
    opt_Rs_tilde_i    = zeros(nImages,nImages);
    opt_thetaij_i     = zeros(nImages,nImages);
    max_corrs_stats_i = zeros(nImages,nImages);
    msg=[];
...
%          npf_i = gpuArray(single(npf_i));
    ...
    for scale_index=1:Nscales
        ...
        scaled_mag_coeff = imresize(npf_i, size(npf_i).*[M, 1]);
        scaled_mag_coeff = gpuArray(scaled_mag_coeff);
        if M<1.0
            scale_coeff = zeros(size(npf_i),'gpuArray');
            scale_coeff(end-size(scaled_mag_coeff,1)+1:end,:)=scaled_mag_coeff;
        else
            scale_coeff = scaled_mag_coeff(end-n_r+1:end,:);
        end
        
        scale_coeff = bsxfun(@rdivide,scale_coeff,...
            sqrt(sum((abs(scale_coeff)).^2)));
         g_npf_i = (single(scale_coeff));
 
         g_npf_i_shifted_scaled = zeros([n_r,n_theta/2,nshifts],'gpuArray');
%          g_npf_i_shifted_scaled = zeros([n_r,n_theta/2,nshifts]);

        % compute all shifted versions of the image
        
        for s=1:nshifts
            g_npf_i_shifted_scaled(:,:,s) = bsxfun(@times,g_npf_i,g_shift_phases(:,s));
        end
        
        g_npf_i_shifted_scaled = reshape(g_npf_i_shifted_scaled,n_r,n_theta/2*nshifts);
 ...
 %       g_scores_self_corrs_i = (single(scores_self_corrs(:,i)));
%         %% test
%         Corrs1 = g_npf_i_shifted_scaled' * npf_stack(:,n_theta*i+1:end); % Corrs is of dimension ( n_theta/2*nshifts x [(nImages - i ) * n_theta])
%         Corrs2=reshape(Corrs1,n_theta^2/2,nshifts,nImages-i);
%         Corrs3 = squeeze(max(real(Corrs2),[],2)); % max out the shifts (recall that any line in fourier space may have a different shift)
%         Corrs4 = reshape(Corrs3,n_theta,n_theta/2,nImages-i);
%         Corrs5 = permute(Corrs4,[2 1 3]);
...
%            g_scores_self_corrs_j = (single(scores_self_corrs(:,j)));
            
            npf_j = npf(:,:,j);
             g_npf_j = gpuArray(single(npf_j));
%            g_npf_j = (single(npf_j));
            
            Corrs = g_npf_j'*g_npf_i_shifted_scaled; % Corrs is of dimension (n_theta x n_theta/2*nshifts )
            ...
            [YY,II] = max(Corrs(:));
            if YY > max_corrs_stats_i(i,j)
                [II,JJ] = ind2sub([nRis_tilde,nRis_tilde],II);
                
                KK = KK(II,JJ);
                
                opt_Rs_tilde_i(i,j)    = gather(II);
                opt_Rs_tilde_i(j,i)    = gather(JJ);
                opt_thetaij_i(i,j)     = gather(KK);
                max_corrs_stats_i(i,j) = gather(YY);
            end
            if scale_index == Nscales
                Ri_tilde = Ris_tilde(:,:,opt_Rs_tilde_i(i,j));
                Rj_tilde = Ris_tilde(:,:,opt_Rs_tilde_i(j,i));
                R_theta_ij = R_theta_ijs(:,:,opt_thetaij_i(i,j));
                ...
                vijs_i((i-1)*3+1:i*3,(j-1)*3+1:j*3)     = vij/n_symm;
                viis_tmp_i((i-1)*3+1:i*3,(j-1)*3+1:j*3) = vii/n_symm;
                viis_tmp_i((j-1)*3+1:j*3,(i-1)*3+1:i*3) = vjj/n_symm;
            end
            ...
            if 1
                ...
                bs = char(repmat(8,1,numel(msg)));
                fprintf('%s',bs);
                msg = sprintf('i=%3d/%3d j=%3d/%3d t=%7.5f\n',i,nImages,j,nImages,t);
                fprintf('%s',msg);
            end
        end
        
    end
    viis_tmp = viis_tmp + viis_tmp_i; % each pair of images will induce a candidate vi^vi^{T}. We keep them all and later average them all out
    vijs = vijs + vijs_i;
    opt_Rs_tilde = opt_Rs_tilde + opt_Rs_tilde_i;
    opt_thetaij = opt_thetaij + opt_thetaij_i;
    max_corrs_stats = max_corrs_stats + max_corrs_stats_i;
    viis_tmp_i = []; 
    vijs_i = [];
    opt_Rs_tilde_i    =[];
    opt_thetaij_i     = [];
...
function [Ris_tilde,R_theta_ijs] = generate_cand_rots(nPoints_sphere,inplane_rot_res,is_use_cands,dir_cand)

vis = generate_vis(nPoints_sphere,is_use_cands,dir_cand);
...
function vis = generate_vis(nPoints_sphere,is_use_dir_cand,dir_cand)
...
if ~is_use_dir_cand
    ...
    nImages = size(dir_cand,2);
    vis_gt = zeros(nImages,3);
    for i=1:nImages
        rot_gt = q_to_rot(dir_cand(:,i)).';
        vis_gt(i,:) = rot_gt(3,:);
    end
    ...
    %%
    nImages = size(dir_cand,1);
    nPoints_sphere_near_cand = round(nPoints_sphere/nImages*0.9);
    nPoints_sphere_rand = nPoints_sphere - nPoints_sphere_near_cand*nImages;
    vis_pert = zeros(nImages*nPoints_sphere_near_cand,3);
    for i=1:nImages
        vis_pert((i-1)*nPoints_sphere_near_cand+1:(i-1)*nPoints_sphere_near_cand+nPoints_sphere_near_cand,:) = repmat(dir_cand(i,:),nPoints_sphere_near_cand,1);
    end
    noise = randn(size(vis_pert));
    
    for i=1:size(noise,1)
        noise(i,:) = 0.1 * noise(i,:)./norm(noise(i,:));
    end
    vis_pert = vis_pert + noise;
    % add some pertrubation to each row
    for i=1:size(noise,1)
        vis_pert(i,:) = vis_pert(i,:)./norm(vis_pert(i,:));
    end

    vis_rand = zeros(nPoints_sphere_rand,3);
    for i=1:size(vis_rand,1)
        tmp = randn(3,3); [Q,~] = qr(tmp);
        vis_rand(i,:) = Q(3,:);
    end
    if 0
    figure;
plot_view_dirs(dir_cand(1:4,:)','blue');
hold on;
plot_view_dirs(vis_pert(1:nPoints_sphere_near_cand*4,:)','red');
hold on;
plot_view_dirs(vis_rand(1:9,:)','green');
    end
    vis = [dir_cand ; vis_pert; vis_rand];
end
...
function [cijs_inds,Ris_tilde,R_theta_ijs,n_theta] = compute_cls_inds(Ris_tilde,R_theta_ijs,n_theta)
