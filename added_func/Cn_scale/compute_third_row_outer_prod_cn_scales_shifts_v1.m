function [vijs,viis,max_corrs_stats] = ...
    compute_third_row_outer_prod_cn_scales_shifts...
...
%     Corrs_scls = zeros(size(ciis_inds),'gpuArray');
    ...
    tmp_Corrs(cii_equators_inds) = -inf;
    
    tmp_Corrs = real(tmp_Corrs);
    [Y,I] = max(tmp_Corrs);
    ...
    % output variable
    scores_self_corrs(:,i) = gather(tmp_Corrs);
...
%         Corrs = gpuArray(zeros(n_theta*n_theta/2,nshifts,Nscales));
        Corrs = (zeros(n_theta*n_theta/2,nshifts,Nscales));
       
for scale_index=1:Nscales
            M=scales(scale_index);
            scaled_mag_coeff = imresize(npf_j, size(npf_j).*[M, 1]);
            if M<1.0
                scale_coeff = zeros(size(npf_j));
...
%             g_npf_j = gpuArray(single(scale_coeff));
            g_npf_j = (single(scale_coeff));
           
% g_npf_j_shifted_scaled = zeros([n_r,n_theta/2,nshifts],'gpuArray');
g_npf_j_shifted_scaled = zeros([n_r,n_theta/2,nshifts]);

            for s=1:nshifts
                g_npf_j_shifted_scaled(:,:,s) = bsxfun(@times,g_npf_j,g_shift_phases(:,s));
            end
            
            g_npf_j_shifted_scaled = reshape(g_npf_j_shifted_scaled,n_r,n_theta/2*nshifts);
            
            tmp_Corrs = g_npf_i'*g_npf_j_shifted_scaled; % Corrs is of dimension (n_theta x n_theta/2*nshifts )
            
            tmp_Corrs = reshape(tmp_Corrs,n_theta*n_theta/2,nshifts);
            Corrs(:,:,scale_index) = tmp_Corrs;
            
            
        end
        Corrs = max(real(Corrs),[],3); % max out the scales (recall that any line in fourier space may have a different scale)
