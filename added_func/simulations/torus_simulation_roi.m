...
vol = zeros(im_size,im_size,im_size);
zmax = 2;
dr = sin(linspace(0,pi,length(-zmax:zmax)));
R = 30;
r=20;
m=0;
for i=-zmax:zmax
    
     m = X>0& X.^2+Y.^2 < R^2+ r^2*dr(i+zmax+1) & X.^2+Y.^2 > R^2 - r^2*dr(i+zmax+1) ;
      m = m |  (Y > R) & X >-10 & X < 10;
    vol(:,:,x0+i) = m;
end
figure;vol3d('Cdata',vol);view(3);
...
v_GSPR=real(v_GSPR); v_GSPR(v_GSPR<0)=0;
            save(['simulation_est\unsym2_thin_torus_estimation_LUD_scale' num2str(max_scale)],...
...
temp_vol=v_GSPR;
    padlen = (size(temp_vol,1) - size(vol,1))/2;
    p = padarray(vol,[padlen,padlen,padlen]);
    ...
    for s1=-30:30
        for s2=-30:30
            for s3=-30:30
                ...
                new_eps = (1/sum(vol(:))) * sum(abs(vol(:)-v_new_registered(:)));
                if new_eps<eps
...
%     end
            ...
            save(['simulation_est\unsym2_torus_estimation_LUD_scale' num2str(max_scale)],...
