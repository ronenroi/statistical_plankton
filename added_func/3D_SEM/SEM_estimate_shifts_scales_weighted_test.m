...
function [est_shifts,shift_equations,est_scales,scale_equations,corrstack]=SEM_estimate_shifts_scales_weighted_test(pf,clstack,...
...
%W_vec(shift_equation_idx) = W(idxi,idxj)/n_projs;
    ...
    cij = clstack(idxi,idxj);
    cji = clstack(idxj,idxi);
%     Ri=rotations(:,:,idxi);
%     Rj=rotations(:,:,idxj);
