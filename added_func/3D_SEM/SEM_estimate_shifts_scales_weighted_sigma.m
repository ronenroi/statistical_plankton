function [clstack,est_shifts,shift_equations,est_scales,scale_equations,corrstack]=SEM_estimate_shifts_scales_weighted_sigma(pf,rotations,...
    max_shift,n_shifts,max_scale,Nscales,sigma,memoryfactor,shifts_2d_ref,verbose)
%
...
for shift_equation_idx=1:Nequations
    ...
    % larger than PI.
     cl_i_vec = mod(cij-sigma : cij+sigma,n_theta) ;
     cl_j_vec = mod(cji-sigma : cji+sigma,n_theta) ;
     cl_j_vec(cl_i_vec>=n_theta/2)=cl_j_vec(cl_i_vec>=n_theta/2)-n_theta/2;
     cl_j_vec(cl_j_vec<0) = cl_j_vec(cl_j_vec<0) + n_theta;

    cl_i_vec(cl_i_vec>=n_theta/2)=cl_i_vec(cl_i_vec>=n_theta/2)-n_theta/2;
    
    
    cl_i_vec=cl_i_vec+1; cl_j_vec=cl_j_vec+1; % Since commonline_R returns zero-based indices.
    
    % Extract the Fourier rays that correspond to the common line.
    pfi=pf(:,cl_i_vec,idxi);
    
    isPfjFlipped=zeros(size(cl_i_vec));  % Is the common line in image j in the positive
    % direction of the ray (isPfjflipped=0) or in the
    % negative direction (isPfjflipped=1).
    cl_j_vec2 = cl_j_vec;
    cl_j_vec2(cl_j_vec<=n_theta2)=cl_j_vec(cl_j_vec<=n_theta2);
    cl_j_vec2(cl_j_vec>n_theta2)=cl_j_vec(cl_j_vec>n_theta2)-n_theta2;
    pfj=pf(:,cl_j_vec2,idxj);
    isPfjFlipped(cl_j_vec>n_theta2)=1;
    
    % Find 1D shift between pfi and pfj.
    pfi=bsxfun(@times,pfi,H);
    pfi(rmax:rmax+2,:)=0;
    pfi=cryo_raynormalize(pfi);
    pfi=pfi(1:rmax,:);  % Take half ray plus the DC
    
    pfj=bsxfun(@times,pfj,H);
    pfj(rmax:rmax+2,:)=0;
    ...
    for scale_index=1:Nscales
        ...
        for shiftidx=1:n_shifts
            ...
            C1=2*real(pfi_stack'*pfj_scaled);
        ...
        if max(sval1,sval2) > sval
            ...
            [cli,clj]=ind2sub([length(cl_i_vec) length(cl_i_vec)],tmp_i);
                    cl1 = cl_i_vec(cli);
                    cl2 = cl_j_vec(clj);
                    isflip = isPfjFlipped(clj);
                    clstack(idxi,idxj)=cl1;
                    clstack(idxj,idxi)=cl2;
    ...
    shift_alpha=(cl1-1)*dtheta;  % Angle of common ray in projection i.
    shift_beta= (cl2-1)*dtheta;  % Angle of common ray in projection j.
